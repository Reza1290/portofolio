...
const StackComponent: React.FC<StackComponentProps> = ({ id, name, imageUrl, first, last, type }) => {
    return (
        ...
        >
            ...
            <Image width={'160'} height={'160'} className={`shrink-0 w-40 h-40 rounded-md shadow-xl bg-white ${first && !last ? 'mask-first' : !first && last ? 'mask-last' : ''}`} src={"/" +imageUrl} alt="Radio Thumbnail" />
        </label>
    );
};
